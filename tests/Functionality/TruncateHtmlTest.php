...
final class TruncateHtmlTest extends TestCase
{
    ...
    {
        $truncate = new TruncateHtml(new ByteStrategy());
        $result = $truncate->execute($this->getHtml(), $limit);
    ...
    #[DataProvider('getDataByteChar')]
    public function testByteChar(int $limit, string $expected): void
    {
        $truncate = new TruncateHtml(new ChartStrategy());
        $result = $truncate->execute($this->getHtml(), $limit);
     ...
     * @return iterable<string, array{int, string}>
     */
    public static function getDataByteChar(): iterable
    {
